    ...
    <tbody>
      ...
      <xsl:for-each select="S3:ListBucketResult/S3:Contents">
        <tr>
          ...
          <td><a href="#" onclick="view(this.getAttribute('key')); return false"><xsl:attribute name="key"><xsl:value-of select="S3:Key"/></xsl:attribute><xsl:value-of select="S3:Key"/></a></td>
